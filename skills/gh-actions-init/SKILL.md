...
**Staging** is decided by one probe — does `stage` exist on the remote? No `stage` branch → no staging environment, full stop (the default; report it as `n/a`). `stage` present → a second release-please instance cutting pre-release tags, per §7. Never propose a staging service that watches a branch.
...
**Staging: only if the repo has a `stage` branch — and then it is tagged too.** Detect with `git ls-remote --heads origin stage`.
