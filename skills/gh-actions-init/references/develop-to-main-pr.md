...
- This does not replace release-please. The chain is: merge `develop → main` (this PR) → release-please opens a versioned release PR → that PR auto-merges once its own checks pass → the merge tags `vX.Y.Z` and, in the same workflow run, deploys the tagged commit. **Merging this promotion PR is the only human gate in the whole release** — see `references/tagged-deploy.md`.
